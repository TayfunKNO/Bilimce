const translateToEnglish = async (query) => {
  try {
    ...
    const res = await fetch(`https://api.mymemory.translated.net/get?q=${encoded}&langpair=tr|en`)
...
export async function searchPubMed(query, limit = 50) {
  try {
    ...
    const searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(englishQuery)}&retmax=${limit}&retmode=json&sort=pub+date`
    ...
    for (const article of articleMatches) {
      ...
      const year = article.match(/<PubDate>[\s\S]*?<Year>(\d{4})<\/Year>/)?.[1] || 
                   article.match(/<ArticleDate>[\s\S]*?<Year>(\d{4})<\/Year>/)?.[1] || ''
      const month = article.match(/<PubDate>[\s\S]*?<Month>(\w+)<\/Month>/)?.[1] || '01'
      ...
      if (pubmedId && title) {
        articles.push({
          ...
          journal,
          published_date: year ? `${year}-${month}-01` : '',
