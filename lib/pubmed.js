...
const extractKeywords = async (query) => {
  try {
    ...
    const stopWords = [
      'related', 'research', 'about', 'regarding', 'concerning', 'studies', 'study',
      'articles', 'article', 'papers', 'paper', 'information', 'with', 'and', 'the',
      'for', 'of', 'in', 'on', 'a', 'an', 'to', 'is', 'are', 'was', 'were',
      'increase', 'improvement', 'enhancement', 'development', 'training',
      'related to', 'about the', 'for the', 'of the'
    ]

    let cleaned = translated.toLowerCase()
    ...
    // Eğer temizlendikten sonra çok kısa kaldıysa orijinal çeviriyi kullan
    if (cleaned.length < 5) return translated
...
export async function searchPubMed(query, limit = 50) {
  ...
  try {
    ...
    const searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(keywords)}&retmax=${limit}&retmode=json`
