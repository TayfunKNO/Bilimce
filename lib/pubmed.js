const buildPubMedQuery = async (query) => {
  try {
    ...
    // Gereksiz kelimeleri temizle — sadece anahtar kelimeler kalsın
    const stopWords = [
      'related', 'regarding', 'concerning', 'about', 'with', 'the', 'for',
      'of', 'in', 'on', 'a', 'an', 'to', 'is', 'are', 'was', 'were',
      'research', 'studies', 'articles', 'information', 'papers', 'paper',
      'and', 'or', 'its', 'their', 'this', 'that', 'which', 'how',
      'increase', 'improvement', 'enhancement', 'development', 'effects',
      'effect', 'role', 'use', 'using', 'used'
    ]

    let words = translated.toLowerCase().split(/\s+/)
    words = words.filter(w => w.length > 2 && !stopWords.includes(w))
    
    // En fazla 3 anahtar kelime al
    const result = words.slice(0, 3).join(' ')
    return result.length > 2 ? result : translated
  } catch {
    return query
  }
}
