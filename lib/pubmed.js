...
const buildPubMedQuery = async (query) => {
  try {
    ...
    Object.entries(corrections).forEach(([tr, en]) => {
      translated = translated.replace(new RegExp(tr, 'gi'), en)
    })

    translated = translated
      .replace(/\b(related|regarding|concerning|with|the|for|of|in|on|a|an|to|is|are|was|were|research|studies|articles|information|about)\b/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim()

    const words = translated.split(' ').filter(w => w.length > 2).slice(0, 4)
    const result = words.length > 0 ? words.join(' ') : query
    
    console.log('Original query:', query)
    console.log('Translated:', translated)
    console.log('PubMed query:', result)
