...
export async function generateMetadata({ params }) {
  const { data } = await supabase
    .from('articles')
    .select('title_tr, title_en, abstract_tr, abstract_en, journal')
    .eq('pubmed_id', params.id)
    .single()

  const title = data?.title_tr || data?.title_en || 'BİLİMCE Makale'
  const description = (data?.abstract_tr || data?.abstract_en || '').slice(0, 160)
        ...
        {!article ? (
          ...
          <article>
            <div className="mb-8">
              ...
              {article.title_tr && (
                <p className="text-white/40 text-base leading-snug mb-4">{article.title_en}</p>
              )}
              <div className="flex flex-wrap gap-3 text-xs text-white/40">
                {article.journal && (
                  <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg">{article.journal}</span>
                )}
                {article.published_date && (
                  <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg">{article.published_date.slice(0,4)}</span>
                )}
                {article.authors && (
                  <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg">{article.authors}</span>
                )}
            ...
            {abstract ? (
              <div className="bg-white/3 border border-white/5 rounded-2xl p-6 mb-6">
                <h2 className="text-sm font-semibold text-white/50 uppercase tracking-wide mb-4">Özet</h2>
                {sections.length > 1 ? (
            ...
            <div className="flex gap-3">
              <a
                ...
                className="px-6 py-3 bg-blue-500/20 border border-blue-500/20 text-blue-300 rounded-xl text-sm font-medium hover:bg-blue-500/30 transition"
              >
                PubMed'de Görüntüle →
              </a>
              <a
                href="/"
                className="px-6 py-3 bg-white/5 border border-white/10 text-white/60 rounded-xl text-sm hover:text-white transition"
              >
                ← Geri Dön
              </a>
            </div>
