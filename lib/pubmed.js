const buildPubMedQuery = async (query) => {
  try {
    ...
    // Her kelimeyi Title/Abstract filtresiyle sorgu oluştur
    const pubmedQuery = words.map(w => `${w}[Title/Abstract]`).join(' AND ')
    return pubmedQuery
  } catch {
    return query
  }
...
export async function searchPubMed(query, limit = 50) {
  try {
    ...
    // Sonuç az geldiyse filtresiz dene
    if (ids.length < 10) {
      const simpleQuery = pubmedQuery.replace(/\[Title\/Abstract\]/g, '').replace(/ AND /g, ' ')
      const fallbackUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(simpleQuery)}&retmax=${limit}&retmode=json`
