const buildPubMedQuery = async (query) => {
  try {
    ...
    const pubmedQuery = words.map(w => `${w}[Title]`).join(' AND ')
    return pubmedQuery
  } catch {
    return query
  }
...
export async function searchPubMed(query, limit = 50) {
  try {
    ...
    if (ids.length === 0) {
      const fallbackQuery = words => words.map(w => `${w}[Title/Abstract]`).join(' AND ')
      const simpleQuery = pubmedQuery.replace(/\[Title\]/g, '[Title/Abstract]')
