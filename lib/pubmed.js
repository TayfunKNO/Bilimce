const cache = {}

const buildPubMedQuery = async (query) => {
  try {
    // İngilizceye çevir
    const encoded = encodeURIComponent(query.slice(0, 490))
    const res = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=en&dt=t&q=${encoded}`)
    const data = await res.json()
    const translated = data[0]?.map(t => t[0]).filter(Boolean).join('') || query

    // Gereksiz ifadeleri temizle
    const noise = [
      /related (research|studies|articles|papers)/gi,
      /research (about|on|regarding)/gi,
      /studies (about|on|regarding)/gi,
      /information (about|on)/gi,
      /\b(related|regarding|concerning|about|with|and|the|for|of|in|on|a|an|to|is|are|was|were)\b/gi,
    ]

    let cleaned = translated
    noise.forEach(pattern => { cleaned = cleaned.replace(pattern, ' ') })
    cleaned = cleaned.replace(/\s+/g, ' ').trim()

    // Kelimeleri ayır ve PubMed sorgusu oluştur
    const words = cleaned.split(' ').filter(w => w.length > 2)
    
    if (words.length === 0) return translated
    if (words.length === 1) return `${words[0]}[Title/Abstract]`
    
    // İlk 4 kelimeyi Title/Abstract filtreyle birleştir
    const queryParts = words.slice(0, 4).map(w => `${w}[Title/Abstract]`)
    return queryParts.join(' AND ')
  } catch {
    return query
  }
}

export async function searchPubMed(query, limit = 50) {
  const cacheKey = `${query}_${limit}`
  if (cache[cacheKey]) return cache[cacheKey]

  try {
    const pubmedQuery = await buildPubMedQuery(query)
    console.log('PubMed query:', pubmedQuery)
    
    const searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(pubmedQuery)}&retmax=${limit}&retmode=json`
    const searchRes = await fetch(searchUrl)
    const searchData = await searchRes.json()
    let ids = searchData.esearchresult?.idlist || []
    
    // Eğer sonuç az geldiyse daha geniş sorgu dene
    if (ids.length < 5) {
      const words = pubmedQuery.replace(/\[Title\/Abstract\]/g, '').replace(/ AND /g, ' ').trim()
      const fallbackUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(words)}&retmax=${limit}&retmode=json`
      const fallbackRes = await fetch(fallbackUrl)
      const fallbackData = await fallbackRes.json()
      ids = fallbackData.esearchresult?.idlist || ids
    }

    if (ids.length === 0) return []

    const fetchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=${ids.join(',')}&retmode=xml`
    const fetchRes = await fetch(fetchUrl)
    const xml = await fetchRes.text()

    const articles = []
    const matches = xml.match(/<PubmedArticle>[\s\S]*?<\/PubmedArticle>/g) || []

    for (const article of matches) {
      const pubmedId = article.match(/<PMID[^>]*>(\d+)<\/PMID>/)?.[1]
      const title = article.match(/<ArticleTitle[^>]*>([\s\S]*?)<\/ArticleTitle>/)?.[1]?.replace(/<[^>]+>/g, '') || ''
      const abstractSection = article.match(/<Abstract>([\s\S]*?)<\/Abstract>/)?.[1] || ''
      const abstractParts = []
      const abstractTextMatches = abstractSection.match(/<AbstractText[^>]*>([\s\S]*?)<\/AbstractText>/g) || []
      for (const part of abstractTextMatches) {
        const label = part.match(/Label="([^"]+)"/)?.[1]
        const text = part.replace(/<[^>]+>/g, '').trim()
        if (text) abstractParts.push(label ? `${label}: ${text}` : text)
      }
      const abstract = abstractParts.join('\n\n')
      const journal = article.match(/<Title>([\s\S]*?)<\/Title>/)?.[1] || ''
      const year = article.match(/<PubDate>[\s\S]*?<Year>(\d+)<\/Year>/)?.[1] || ''
      const lastNames = article.match(/<LastName>([\s\S]*?)<\/LastName>/g)?.slice(0, 3).map(n => n.replace(/<[^>]+>/g, '')) || []

      if (pubmedId && title) {
        articles.push({
          pubmed_id: pubmedId,
          title_en: title,
          abstract_en: abstract,
          journal,
          published_date: year,
          authors: lastNames.join(', '),
          source: 'pubmed',
        })
      }
    }

    articles.sort((a, b) => (parseInt(b.published_date) || 0) - (parseInt(a.published_date) || 0))
    cache[cacheKey] = articles
    return articles
  } catch (err) {
    console.error('PubMed error:', err)
    return []
  }
}
